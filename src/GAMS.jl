...
const GAMS_MODEL_STATUS_NO_SOLUTION = (
    MODEL_STATUS_ERROR_UNKNOWN,
    MODEL_STATUS_ERROR_NO_SOLUTION,
    MODEL_STATUS_NO_SOLUTION_RETURNED,
    MODEL_STATUS_UNBOUNDED_NO_SOLUTION,
    MODEL_STATUS_INFEASIBLE_NO_SOLUTION,
    MODEL_STATUS_INTEGER_INFEASIBLE,
...
const GAMS_MODEL_ATTRIBUTES = union(GAMS_MODEL_ATTRIBUTES_INT, GAMS_MODEL_ATTRIBUTES_REAL)

# required GAMS model attributes
const GAMS_MODEL_ATTRIBUTES_REQUIRED = ("modelStat", "numEqu", "numVar", "solveStat")

model_type_from_label(mtype::String) =
    if mtype == "LP"
...
GAMSSolutionSymbol(n_records::Int) =
    GAMSSolutionSymbol(n_records, zeros(n_records), zeros(n_records))
...
GAMSSolution() =
    GAMSSolution(Dict{String, GAMSSolutionSymbol}(), Dict{String, GAMSSolutionSymbol}())
...
mutable struct GAMSWorkspace
    ...
    function GAMSWorkspace(working_dir::String, system_dir::String)
        check_system_dir(system_dir)
        return new((0, 0, 0), working_dir, system_dir)
    end
end

function GAMSWorkspace(system_dir::String)
    working_dir = mktempdir(prefix = "gams_jl_")
    return GAMSWorkspace(working_dir, system_dir)
end
...
function GAMSWorkspace()
    ...
    return GAMSWorkspace(splitdir(path)[1])
end

function check_system_dir(system_dir::String)
    if Sys.iswindows()
...
set_system_dir(workspace::GAMSWorkspace, system_dir::String) =
    if check_system_dir(system_dir)
        workspace.system_dir = system_dir
        workspace.version = (0, 0, 0)
    end

function set_working_dir(workspace::GAMSWorkspace, working_dir::String)
    rm(workspace.working_dir, force = true, recursive = true)
    if !ispath(working_dir)
        mkpath(working_dir)
    end
    return workspace.working_dir = working_dir
end

function get_version(workspace::GAMSWorkspace)
...
function run(job::GAMSJob; options::Dict{String, Any}, solver_options::Dict{String, Any})
    ...
    open(job.filename, "a") do io
        ...
        return Base.write(io, "execute_unload '$(job.jobname)_stats.gdx', attr, stats;\n")
    end
